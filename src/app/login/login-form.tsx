...
export default function LoginForm({ initialSettings }: LoginFormProps) {
  ...
  const schoolName = initialSettings.schoolName || "ระบบจัดการการลา";
  const subheader = initialSettings.subheader || "ระบบจัดการการลา";
  const logoUrl = initialSettings.logoUrl;
  const footerText = initialSettings.footerText || "© 2006 Panchapon Getrat KP-school";
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#F4F7FB] dark:bg-slate-900 relative overflow-hidden p-4">
        ...
        <div className="flex flex-col items-center max-w-sm w-full text-center space-y-6 relative z-10 animate-in fade-in zoom-in duration-500">
          {logoUrl ? (
            ...
            <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-purple-500 to-indigo-650 flex items-center justify-center shadow-2xl shadow-purple-500/20 mb-2">
              <Lock className="w-10 h-10 text-white" />
            </div>
          )}
          
          <div className="space-y-2">
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{schoolName}</h1>
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{subheader}</p>
          </div>

          <div className="w-48 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden relative">
            <div className="h-full bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full w-full animate-loading-bar absolute left-0 top-0 origin-left" />
          </div>
        </div>
      </div>
    );
