              ...
              >
                ...
                {notifications.length > 0 && (
                  <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 shrink-0">
                    <Link
                      href={counts.leaves > 0 ? "/eleave/approvals" : "/eleave/users"}
                      onClick={() => setNotiOpen(false)}
...
function AppContent({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/eleave/dashboard", label: t("dashboard"), icon: LayoutDashboard },
    { href: "/eleave/request", label: t("requestLeave"), icon: FileText },
    { href: "/eleave/history", label: t("history"), icon: History },
  ];

  if (isApprover) {
    navItems.push({ href: "/eleave/approvals", label: t("approvals"), icon: CheckSquare });
  }
  
  if (isAdmin) {
    navItems.push(
      { href: "/eleave/reports", label: t("reports"), icon: FileSpreadsheet },
      { href: "/eleave/users", label: t("users"), icon: Users },
      { href: "/eleave/logs", label: t("logs"), icon: Activity },
      { href: "/eleave/settings", label: t("settings"), icon: Settings }
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-[#F4F7FB] dark:bg-slate-900 text-slate-900 dark:text-white font-sans selection:bg-purple-500/30">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[280px] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200/50 dark:border-slate-800/50 flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] shadow-[4px_0_24px_rgba(0,0,0,0.02)] print:hidden ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        {/* Back to School OS Home Button */}
        <div className="px-6 pb-4">
          <Link href="/">
            <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 shadow-sm transition-all duration-300 group cursor-pointer">
              <svg className="w-4 h-4 text-indigo-500 group-hover:-translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
              <span>กลับสู่หน้าหลัก School OS</span>
            </div>
          </Link>
        </div>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          <Link href="/eleave/profile">
            <div className={`relative flex items-center gap-3 px-4 py-3 rounded-2xl text-[14.5px] font-medium transition-all duration-300 group overflow-hidden ${
              pathname === "/eleave/profile" 
                ? "text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-500/10" 
                : "text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50"
            }`}>
              {pathname === "/eleave/profile" && (
              ...
              <UserCircle className={`w-5 h-5 transition-transform duration-300 ${pathname === "/eleave/profile" ? "scale-110" : "group-hover:scale-110"}`} />
              {t("profile")}
            </div>
          </Link>
        </nav>
